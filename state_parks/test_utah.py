...
def sns_publish(topic_arn, event_type, message, attributes=None, compress=False):
    if not attributes:
        attributes = {}
        attributes['event_type'] = {"DataType": "String", "StringValue": event_type}
    ...
    if compress:
        ...
        attributes["method"] = {"DataType": "String", "StringValue": "compressed"}

    sns = boto3.resource("sns")
    client = sns.Topic(topic_arn)
    message_id = client.publish(Message=message,
                                MessageAttributes=attributes)
    logger.info("Publish to SNS returned Message ID: {}".format(message_id))
    return message_id
...
# def get_caps(park):
#     time = datetime.strftime(datetime.today(), "%Y %m s%d - %H:%M")
#     desired_cap = {
#         'os' : 'Windows',
#         'os_version' : '10',
#         'browser' : 'Chrome',
...
def format_reservations(driver, state, config):
    ...
    for reservation in reservation_list:
        ...
        try:
            ...
            logger.warn(f"ALERT! For reservation {res_number}")
            time.sleep(3)
...
def main():
    ...
    event = {
        ...
        "format": "dropdown_stations"
    }
    config = {
        "GRID_URL": "https://tentrrengineerin1:rAtJjVAYxEJRTRfiGHzH@hub-cloud.browserstack.com/wd/hub",
        "SNS_TOPIC_ARN": "arn:aws:sns:us-east-1:192994485053:tentrr-core-prod",
        "UT_USER": "ut-tenter",
        "UT_PASS": "password3",
        "LA_USER": "la-cfarco",
        "LA_PASS": "park1234"
